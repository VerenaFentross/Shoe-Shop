...
let accept = () => {
  ...
  //   document.cookie = "username = visitor; max-age = 100000";

  // on reload kein banner??
};
...
const regal = document.getElementById("shoeDisplay");

data.forEach((shoeObject) => {
  let shoebox = document.createElement("div");
  regal.appendChild(shoebox);

  let image = document.createElement("img");
  image.src = shoeObject.img;
  shoebox.appendChild(image);

  //ACCORDEON (NICHT)

  let shoeStats = document.createElement("div");
  shoeStats.innerHTML = `<button class="accordion"><h4> ${shoeObject.name}</h4></button><div class="panel"<p>${shoeObject.price}</p><p>${shoeObject.brand}</p><p>${shoeObject.sizes}</p></div>`;
  shoebox.appendChild(shoeStats);
